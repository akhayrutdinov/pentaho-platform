...
public class SessionCachingMetadataDomainRepositoryTest extends BaseTest {
  ...
  public void testCreate_no_delegate() throws Exception {
    try {
      ...
      fail(
        "Should not be able to create a Session Caching Repository without a base implementation (delegate)" );
        //$NON-NLS-1$
  ...
  public void testGetDomainIds() throws Exception {
    ...
    PentahoSessionHolder
      .setSession( new StandaloneSession( "Standalone Session", "1234-5678-90" ) ); //$NON-NLS-1$ //$NON-NLS-2$
